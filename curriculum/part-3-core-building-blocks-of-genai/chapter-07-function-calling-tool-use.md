...
### The Model Context Protocol, examined

MCP deserves the closer look that one-line reading compresses, because it has become the de-facto standard for the plumbing. The architecture is **host / client / server**: the *host* is the LLM application (the chat assistant, the IDE, the agent runtime — 4.4), it runs an MCP *client* per connection, and each *server* exposes capabilities — **tools** (discoverable and invocable — this chapter's contracts, wire-formatted), **resources** (readable context: files, records, documents), and **prompts** (reusable templates the server offers the host). What the protocol standardizes is precisely the plumbing: how a host *discovers* what a server offers, how it *invokes* a tool and receives results, how capabilities are described — so a tool server written once works across every MCP-speaking host, which is the entire value proposition. What it deliberately does not standardize is everything above: tool granularity and task-shaping, consequence classes and gates, authorization policy, description quality, and whether the server's code deserves your trust — the protocol will transport a god-credentialed, ungated, API-shaped tool to your model with perfect fidelity.

Two deployment shapes with different trust profiles. **Local servers** (spawned as subprocesses, stdio transport) run with the host machine's privileges — convenient for developer tooling, and exactly as trustworthy as the package you installed (supply chain — 4.9). **Remote servers** (HTTP transport) are network services with real authentication and authorization surfaces (OAuth-based — 6.6 details the model) — the enterprise-relevant shape, because they centralize credential handling and can be governed like any service.

The adoption decision is a reuse question, not a fashion question. Reach for MCP when **tool reuse across hosts is real** — the same enterprise tools serving the IDE assistant, the chat assistant, and the agent fleet, written once — or when you're consuming an existing server ecosystem. Keep a **bespoke tool layer** when one application owns its tools: the protocol adds an abstraction hop and a dependency for zero reuse benefit, and your in-process execution layer already does the job with less machinery. And in the enterprise, the control point is the **registry-as-allowlist**: which servers may be connected, at which pinned versions, with which credentials, approved by whom — an unreviewed server is an unassessed capability surface *and* an untrusted content source, because its tool descriptions enter your prompts as trusted-looking text (4.9 names the resulting threat classes: tool poisoning, rug-pull servers, name squatting). The protocol commoditizes connection; your governance decides what deserves connecting.
