...
Token discipline is money with a small delay. The mechanisms from Chapter 2.5 price it: input tokens dominate context-heavy bills (10:1 to 50:1 input:output ratios in RAG-style chat — though reasoning-heavy workloads invert this, with thinking tokens billed as output at several times the input price dominating instead; Chapter 2.6), prefill drives time-to-first-token, and caching pays only for disciplined prompt structure — so an undisciplined context is simultaneously the biggest cost line, the biggest latency line, and a foregone discount. Concrete enterprise shape: a 20-person team's assistant at 5K conversations/day, carrying 8K tokens of unmanaged context per turn versus a budgeted 3K, is paying roughly 2.5× on the dominant cost line for *no measured quality gain* — typically five figures monthly, discovered at budget review. Sampling discipline is risk with a small delay: a temperature left high in an extraction pipeline manufactures variance exactly where the business wanted determinism (Chapter 3.1's variance limit, self-inflicted), and the resulting "the AI is inconsistent" complaint is a configuration bug wearing a capability costume. Both disciplines are free; both defaults are expensive.
