...
- **Precision and recall trade off** via the decision threshold: flag more aggressively and recall rises while precision falls. The threshold is a *business decision wearing a technical costume* — where it sits should be set by the relative cost of misses vs. false alarms (Chapter 1.3's play logic), not by a data scientist's default. F1 (their harmonic mean) and AUC (threshold-free ranking quality) are useful summaries — with one standing caveat: under heavy class imbalance, ROC-AUC flatters, because the flood of easy true negatives dominates it; for rare-event problems (fraud at 1-in-1,000), read precision–recall curves and recall at a fixed false-positive rate instead. But the architect's question in every review is the plain one: *which error hurts us more, and does the metric reflect that?*
