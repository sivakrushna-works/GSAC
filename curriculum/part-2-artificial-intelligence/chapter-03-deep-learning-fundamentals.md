...
- **The sparse refinement (Mixture-of-Experts):** one architectural trick has become common enough at the frontier that the cost story needs an asterisk. A **Mixture-of-Experts (MoE)** model replaces parts of the network with many parallel "experts" plus a learned router that sends each token through only a few of them — so a model can hold a very large *total* parameter count while any single token touches only a fraction of it (its **active parameters**). The consequence rewrites a sentence you were about to internalize: inference compute and latency scale with *active parameters per token*, not headline model size — two models with the same marketing size can differ several-fold in serving cost, and a "small-feeling" fast model may be a huge sparse one underneath. The catch is memory: *all* experts must be loaded to serve, so a sparse model's GPU-memory footprint still tracks total size (Chapter 5.3's serving math). Architect's rule: when a model card gives one parameter number, ask for both — total for memory, active for compute.
...
Consequences you'll apply constantly. **The weights are an artifact, not a service**: versioned, immutable, promoted through environments like any release (Chapter 5.7's LLMOps pins model versions for exactly this reason — a provider swapping weights under a stable API name is a *release you didn't test*, and the deployment checklist's model-pinning line descends from this diagram). **The planes scale independently**: enterprises consuming models via API have outsourced the training plane entirely — the Chapter 2.1 utility shift, drawn as architecture — and their design surface is the inference plane: latency, throughput, cost, fallback (Chapters 5.3–5.9). **Inference is physics-bound**: a forward pass has an irreducible compute cost; latency and unit cost are functions of *active parameters per token* (which sparse-MoE serving decouples from headline model size), which is why model tiering (small models for easy work — Chapter 7.8) is an architectural pattern and not just thrift.
...
- [ ] Latency budget reflects active-parameter physics (total vs. active counts known for candidate models); tiering considered before hardware
...
- Fedus et al., *Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity* (arxiv.org/abs/2101.03961) — the readable case for sparse activation and why "total parameters" stopped being the cost number; the origin idea is Shazeer et al.'s sparsely-gated MoE (arxiv.org/abs/1701.06538), and Mixtral (arxiv.org/abs/2401.04088) is the open-weights proof it went mainstream.
...
- Architecture inherits a **two-plane world**: weights as versioned artifacts, serving as the enterprise design surface, cost as observable physics (tokens × active parameters — sparse MoE having split "active" from "total").
