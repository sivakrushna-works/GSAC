...
4. Specify what the platform team owes tenants for the autonomy they give up: an SLA, a deprecation window, migration support, and a way off the path.

## Introduction

A platform is an economic claim before it is an architecture, and the claim is falsifiable: **the Nth team must be cheaper than the first.** If building on the platform costs a team what building alone would cost — plus the tax of learning its conventions, waiting on its roadmap, and working around the one thing it doesn't support — the platform is a cost centre with a diagram.

The family splits along that claim. **Tenancy patterns** govern what one tenant can do to another: take its data, its throughput, its budget, or its quality. **Platform product patterns** govern what the platform sells and what it owes back. The first group carries the AI-specific hazards, because classic SaaS multi-tenancy guidance was written for CRUD systems — it has good answers for databases and none for a shared retrieval corpus, an exhaustible provider quota, or the fact that quality differs per tenant because each tenant's documents differ.
...
**Two teams with divergent needs is the premature case.** A shared abstraction built over a sample of two generalizes from the two requirement sets you happen to have; it fits neither, both teams route around it, and the third inherits an interface shaped by an accident. *Team Topologies*' thinnest-viable-platform discipline is the answer: ship the least that works and let consumers pull capability in, so the third consumer reveals what is actually common.

The opposite error is slower to notice and larger. Past a handful of systems, per-team infrastructure means per-team gateways with per-team failover bugs, eval harnesses that disagree about what "good" means, guardrails at varying maturity, and a governance question nobody can answer: which models are we using, on whose data, at what cost ([6.9](../part-6-enterprise-architecture/chapter-09-architecture-governance.md)). Multi-tenancy raises the stakes from tidiness to disclosure — a retrieval leak across tenants is an incident the victim cannot detect and you may not be able to bound.
...
- **Context** — one AI capability serving workloads owned by different parties: business units under one legal entity, or external customers under separate contracts.
- **Problem** — how much of the stack do tenants share? Decided by taste, the answer is either unaffordable (everyone gets their own) or undefendable (everyone shares everything).
- **Forces** — unit cost falls with sharing while blast radius rises with it; fleet operability inverts, since one shared stack is one thing to patch and forty dedicated stacks are forty migrations; noisy-neighbour tolerance differs per tenant, because an interactive assistant and a nightly bulk job do not want the same neighbours; and residency rules, contractual isolation clauses, or customer-managed-key requirements can remove rungs before engineering has an opinion.
- **Solution** — treat isolation as three rungs and choose a rung *per resource*, not once for the system. Rung 1, shared everything: one deployment, one index, tenant identity as a filter. Rung 2, shared infrastructure with per-tenant data isolation: one serving fleet, per-tenant indexes, namespaces, or keys. Rung 3, dedicated per-tenant resources: a stack per tenant or tenant group. Each rung is justified by a citable requirement — a regulator's rule, a contract clause, or a measured incident.
- **Structure** — tenant identity established at the edge from the authenticated principal, never a client-supplied field → per-resource rung lookup → filtered or dedicated path; a CI test per boundary asserting a cross-tenant read *fails*.
- **Consequences** — climbing buys a defensible isolation story and regulated-account sellability, and buys an N-stack operations problem: version skew, N upgrades, N incident surfaces, more than a small platform team can carry past a couple of dozen tenants. Rung 1 is cheapest and sits one authorization bug from cross-tenant disclosure, so it survives only with defense in depth, not a single filter. Climbing later is a data migration, so the rung belongs in an [ADR](../../GLOSSARY.md) with the trigger that would force a revisit.
- **Known uses** — the silo / pool / bridge vocabulary in mainstream SaaS architecture guidance and the deployment-stamp or cell-based approach in cloud resilience guidance describe this ladder for non-AI SaaS; customer-managed encryption keys are its standard commercial expression. AI adds the index: [7.7](chapter-07-knowledge-data-patterns.md)'s Tenant Isolation is this pattern applied to the corpus. Worked instance (fictional): Vantora runs rung 1 for its gateway, rung 2 for indexes, rung 3 for EU-entity tenants.
- **Related** — Tenant Isolation ([7.7](chapter-07-knowledge-data-patterns.md)); Per-Tenant Quota & Fair-Share (throughput, not data); security architecture ([6.5](../part-6-enterprise-architecture/chapter-05-security-architecture-zero-trust.md)).

#### Pattern: Per-Tenant Quota & Fair-Share

- **Context** — many tenants sharing one capacity pool: a provider account with account-level limits, reserved throughput, or a self-hosted fleet.
- **Problem** — model capacity is a *shared, exhaustible* resource. One tenant's backfill consumes the account's token-per-minute allowance and every other tenant sees throttling for a fault it did not cause and cannot see.
- **Forces** — utilization (pooled capacity is only cheap when full) vs. protection (headroom held for an idle tenant is capacity wasted); burst tolerance vs. sustained fairness; and the hard fact that provider quotas are enforced at the account level, above any per-tenant accounting you invent.
- **Solution** — enforce two independent limits at the gateway: a per-tenant rate limit sized to the tenant's plan, and a fair-share scheduler allocating *spare* capacity by weight so an idle tenant's share stays usable but reclaimable. Split interactive and bulk traffic into separate queues with different priorities. Reject with an explicit throttling response and retry hint rather than by timing out. A tenant that cannot tolerate contention buys dedicated capacity — the ladder's rung 3 applied to throughput.
...
- **Consequences** — starvation stops and utilization drops, because protection costs headroom. Bulk tenants experience real queueing and must be told so in the SLA rather than discovering it. The platform inherits a capacity-planning duty it cannot delegate, since tenants cannot see the pool — and the priority weights are a commercial decision wearing a scheduler config's clothes, so someone outside engineering must set them.
- **Known uses** — quota and rate-limit enforcement at an API gateway is standard practice, and commercial LLM APIs enforce request- and token-rate limits at the account or organization level with a throttling status clients must back off on. Fair queuing for shared control planes is established: the Kubernetes API server's priority-and-fairness machinery exists so one noisy client cannot exhaust a shared server. Worked instance (fictional): Vantora's overnight enrichment tenant starving its interactive assistants produced its bulk queue.
- **Related** — GenAI Gateway (the enforcement point); Usage Metering/Chargeback (same instrumentation, different purpose); cost & performance patterns ([7.8](chapter-08-cost-performance-patterns.md)).

#### Pattern: Usage Metering/Chargeback

- **Context** — a shared platform whose provider bill arrives as one line and whose tenants consume very differently.
...
- **Forces** — accountability vs. metering fidelity, since token counts are exact but shared infrastructure, cache hits, and platform overhead are allocations rather than measurements; incentive strength vs. adoption, because a chargeback landing before attribution is trusted drives teams off the platform; and the fact that a tenant billed for capacity will demand a say in scheduling it.
...
- **Consequences** — teams that pay optimize, and the first month of showback usually finds an expensive prompt nobody owned. The platform inherits a billing system with its own correctness burden, and any drift between metered and invoiced cost is an argument it loses by default. Chargeback also distorts: tenants cut evaluation runs and logging to shrink their bill, so exclude platform-mandated overhead from the recharge.
- **Known uses** — showback and chargeback are standard FinOps practice built on cloud cost-allocation tagging; model spend presents the same account-level allocation problem and takes the same tag-then-reconcile answer. Worked instance (fictional): Vantora's showback made two hold-out teams' un-amortized costs visible, which is what moved them onto the platform.
- **Related** — cost engineering ([4.11](../part-4-enterprise-genai-systems/chapter-11-cost-engineering.md)); Per-Tenant Quota & Fair-Share; observability ([4.10](../part-4-enterprise-genai-systems/chapter-10-observability.md)) as the record source.
...
- **Forces** — a shared bar (comparable, cheap, one dashboard) vs. per-tenant truth (only the tenant's own golden set reflects its users); tenant effort, since golden sets are tenant work tenants resist, against the platform's inability to write them on the tenant's behalf; and cost, since evaluating every tenant on every change multiplies eval spend by tenant count.
- **Solution** — split machinery from content. The platform owns runners, judges, calibration, gates, dashboards; each tenant owns its golden set, rubric weights, and threshold. Every fleet-wide change — model swap, shared prompt revision, retrieval-default change — runs against every tenant baseline before rollout, and a regression blocks the rollout *for that tenant*, not for everyone. Onboarding includes producing a minimum baseline; a tenant without one gets best-effort quality in writing.
...
- **Known uses** — the machinery/content split is the one every shared CI system makes: central runners and reporting, tests owned by the team. Per-segment rather than aggregate evaluation is standard model-validation practice wherever populations differ ([6.11](../part-6-enterprise-architecture/chapter-11-model-risk-management.md)). Worked instance (fictional): Vantora blocked a model upgrade for the tenant whose regulatory corpus regressed while ten others improved.
- **Related** — Shared Eval Service (the machinery consumed); evaluation systems ([4.7](../part-4-enterprise-genai-systems/chapter-07-evaluation-systems.md)); Central Model Governance (whose upgrades this gates).
...
- **Problem** — cross-cutting concerns re-implemented per application at per-application maturity, with no place to answer an estate-level question or enforce an estate-level rule.
- **Forces** — one control point (enforceable policy, one integration, one place to change providers) vs. one dependency on every request path; central policy vs. per-application flexibility; and the political force that a bypassable gateway is the one applications bypass under deadline.
- **Solution** — a service applications call by *task class* rather than model name, which resolves the model ([3.10](../part-3-core-building-blocks-of-genai/chapter-10-model-selection-benchmarking.md)), authenticates the caller and establishes tenant identity, enforces quota, applies [guardrails](../../GLOSSARY.md) ([4.8](../part-4-enterprise-genai-systems/chapter-08-guardrails-content-safety.md)), serves the cache, emits the metering record, and fails over between providers ([5.9](../part-5-cloud-infrastructure-platform/chapter-09-reliability-engineering.md)). Provider credentials live only here — which is what makes it non-bypassable in practice rather than in policy.
...
- **Problem** — prompts edited in place have no version, diff, rollback, or answer to "what produced this output three weeks ago?" At tenant scale one edit is a fleet-wide change with no staging.
- **Forces** — central governance (versioning, review, audit) vs. team and tenant ownership of content; a single current version (coherent, simple) vs. per-tenant pinning (safe, and a combinatorial support surface); immutability vs. the urge to hotfix a prompt mid-incident.
...
- **Consequences** — rollback becomes seconds rather than archaeology, and the registry becomes a hard runtime dependency that must be cached locally or the availability floor drops again. Per-tenant pinning is the honest option and it accumulates: without a retirement schedule the platform supports a dozen prompt generations, and every model upgrade must be validated against all of them.
- **Known uses** — this is the schema-registry and model-registry construction applied to prompts — immutable versioned artifacts, referenced by identifier, with promotion stages — the shape mainstream data and ML platforms already use for message schemas and trained models. Worked instance (fictional): Vantora pins regulated tenants to prompt versions their compliance team reviewed.
- **Related** — prompt engineering ([3.3](../part-3-core-building-blocks-of-genai/chapter-03-prompt-engineering.md)); LLMOps ([5.7](../part-5-cloud-infrastructure-platform/chapter-07-llmops.md)); Central Model Governance (upgrades invalidate prompt assumptions).
...
- **Forces** — the machinery genuinely amortizes (runners, judge hosting, calibration, dashboards, CI integration) while the content genuinely does not (golden sets and rubrics are domain work); central gates enforce a bar and also make the platform team the reason a release is blocked.
- **Solution** — run evaluation as a service: hosted runners, a judge fleet with a documented calibration programme against human labels, standard report formats, and a CI integration that turns a threshold breach into a failed check on the team's own pipeline ([5.7](../part-5-cloud-infrastructure-platform/chapter-07-llmops.md)). Teams supply golden sets, rubrics, thresholds. Publish calibration results so consumers can judge how far to trust a score.
...
- **Related** — evaluation systems ([4.7](../part-4-enterprise-genai-systems/chapter-07-evaluation-systems.md)); Per-Tenant Eval Baseline (the tenant-scoped consumer); observability ([4.10](../part-4-enterprise-genai-systems/chapter-10-observability.md)).
...
- **Known uses** — model inventories with documented approval and review triggers are standard in model risk management ([6.11](../part-6-enterprise-architecture/chapter-11-model-risk-management.md)); published deprecation policies with fixed minimum support windows are established platform-API practice, the Kubernetes API deprecation policy being the canonical public example of the contract's shape. Worked instance (fictional): Vantora's portfolio and routing policy.
...
- **Problem** — mandates get routed around and unopinionated platforms get ignored; meanwhile a path with no exit becomes a cage, and the first team with a legitimate exception either forks the platform or ships something ungoverned.
- **Forces** — opinionation (a path that decides for you is fast) vs. coverage (every decision made for you excludes a team); governance-by-default vs. the reality that the exception is sometimes correct; and the platform team's finite capacity, which the number of supported variations consumes directly.
- **Solution** — ship a template that provisions a working, compliant system with gateway integration, eval wiring, observability, guardrails, and cost tagging already connected, so a team *starts* governed rather than becoming governed later. Then define the exit: a documented way to opt out of a named layer while staying inside the non-negotiable ones — identity, metering, the gateway — with the deviation recorded in an ADR and reviewed on a schedule. In return the platform commits publicly to an availability and latency SLA, a deprecation window, and migration support performed by the platform rather than announced to tenants.
- **Structure** — template repository or scaffolder → provisioned project with integrations pre-wired → team fills in corpus, prompts, evals → deviation register for escape-hatch users → published SLA and deprecation calendar.
...
flowchart TD
    ...
    GW --> GOV[Central Model Governance<br/>task class to model]
    ...
    LADDER[Tenancy Isolation Ladder<br/>rung chosen per resource] -.governs.-> DATA[Per-tenant corpora and indexes]
    PATH[Golden Path with Escape Hatch] -.provisions.-> T1 & T2
```

Three readings. **The gateway is the only place identity, quota, and metering can be enforced together** — which makes it the keystone, and makes its availability budget stricter than any application it serves. **Isolation is not one decision** — the ladder applies per resource, so one platform can share compute, dedicate indexes, and dedicate whole stacks for the tenants whose contracts demand it. **And quality is per tenant, not per platform** — the baseline gate turns a fleet-wide upgrade from a lottery into a staged rollout, at the cost of putting the slowest tenant on the critical path.

## Real-world Example

**Vantora Systems** (fictional, recurring since [5.10](../part-5-cloud-infrastructure-platform/chapter-10-iac-platform-engineering.md)) arrived at its platform the usual way: eleven teams, six model integrations, no inventory. The platform that consolidated them is this catalog composed — gateway carrying identity, quota, guardrails, and metering; registry pinning prompts per tenant; portfolio routing by task class. Tenancy was mixed by design: shared gateway, per-tenant indexes, dedicated stacks for the EU entity whose residency obligation removed the lower rungs.

Two lessons came from operating it rather than designing it. Throughput first: an overnight enrichment tenant exhausted the account's token allowance and the interactive assistants throttled for two hours, which is what bought the bulk queue and per-tenant limits. Then quality: an upgrade that improved ten tenants regressed the eleventh, whose corpus was dense regulatory text, and only that tenant's own baseline caught it. Adoption stayed voluntary — nine of eleven teams took the golden path because it was faster than starting alone, and the last two moved when showback exposed their unamortized cost. The platform's half of the bargain is written down: an availability SLA, a deprecation window, and platform-performed migration when the platform changes.

## Hands-on Exercise

**Design a two-tenant platform slice, then argue against building it.** ~90 minutes.

1. **Break-even (15 min).** For a platform serving two named teams, estimate build cost, annual operating cost, and per-team duplicated effort avoided. State the consumer count at which it breaks even and write the honest recommendation — including "not yet" if that is what the arithmetic says.
2. **Ladder placement (20 min).** For four resources — gateway, vector index, source-document storage, provider capacity — choose a rung and cite the requirement forcing it. Flag any rung chosen by preference.
3. **The AI-specific three (30 min).** Specify (a) the retrieval isolation test proving a cross-tenant read fails, (b) the quota and fair-share policy including what a throttled bulk tenant experiences, (c) the eval-baseline gate for a model upgrade and who may override it.
4. **The bargain (25 min).** Write the platform's obligations: SLA targets, deprecation window, migration commitment, and the escape hatch — which layer a team may leave, which layers are non-negotiable, how deviations are recorded.
...
Organizationally the platform needs a standing team with a product manager, not a rotation: these patterns create perpetual obligations — capacity planning tenants cannot do, migrations they did not ask for, an approval path that must answer fast. A platform team measured on shipped features will under-invest in migration support, which is precisely the obligation that decides whether tenants trust it. Measure golden-path adoption and time-to-first-compliant-deployment; do not measure mandate compliance, which hidden deviations satisfy.
...
| Capacity | Pooled with fair-share | Dedicated per-tenant capacity | Bursty, uncorrelated demand — pooling is what makes it affordable | A latency commitment that cannot survive a neighbour's spike |
| Cost accountability | Showback | Chargeback | Attribution is new and unverified — build trust first | Attribution reconciles to the invoice and tenants have checked it |
...
| Path enforcement | Golden path with escape hatch | Mandated path | Whenever adoption matters more than uniformity | Only for the non-negotiable layers: identity, metering, gateway |
...
2. **Choosing the isolation rung by preference** — "sharing is cheaper" or "dedicated is safer" with no requirement cited and no ADR, for a decision the size of a data migration.
3. **Treating provider capacity as elastic** — no per-tenant limits, no queue separation, so the first bulk workload throttles every interactive tenant.
4. **Tenant identity taken from the request body** — a client-supplied tenant field is an authorization bypass; identity comes from the authenticated principal at the edge.
...
6. **A prompt change as a fleet-wide edit** — no pinning, no staged rollout, so one revision is an incident across every tenant at once.
7. **Governance that cannot say yes** — an unpublished, slow model-approval path produces shadow provider accounts, not compliance.
8. **Taking autonomy and giving nothing back** — no SLA, no deprecation window, no migration help; tenants respond by vendoring their own copy and the amortization argument dies.

## Best Practices

1. **State the break-even before building** — consumers, costs, and the recommendation the arithmetic supports, including "not yet."
2. **Choose the rung per resource, with a citable requirement** — recorded in an ADR with the trigger for revisiting.
...
5. **Showback before chargeback** — and exclude platform-mandated overhead so tenants are not billed for logging and evaluation you require.
...
- [ ] Break-even stated: consumers, build and operating cost, the point where the Nth team is cheaper than the first
...
1. *"When should an enterprise NOT build an internal AI platform?"* — Strong answers give the break-even reasoning (build plus perpetual operation against duplicated effort avoided), name two teams with divergent needs as the premature case, and cite the thinnest-viable-platform discipline: let the third consumer reveal what is genuinely common.
...
3. *"One tenant's batch job is throttling everyone. What did you get wrong?"* — Strong answers name provider quota as a shared exhaustible resource enforced above your own accounting, then give per-tenant limits, separate interactive and bulk queues, fair-share allocation of spare capacity, and explicit throttling responses; noting that priority weights are a commercial decision earns senior marks.
...
5. *"What does the platform team owe its tenants?"* — Strong answers give the reciprocal bargain: an availability and latency SLA, a stated deprecation window, migration the platform performs rather than announces, and a documented escape hatch — with golden-path adoption as the metric proving the bargain holds.
...
- Public internal-developer-platform practice: golden paths and templated scaffolding (Backstage's software templates being the widely used open implementation).
- Mainstream cloud SaaS-tenancy guidance — silo/pool/bridge isolation models and the deployment-stamp or cell-based approach — for the non-AI half of the ladder.
- FinOps practice on showback versus chargeback and cost-allocation tagging.
- [7.7 Knowledge & Data Patterns](chapter-07-knowledge-data-patterns.md) (Tenant Isolation at the index), [7.8 Cost & Performance Patterns](chapter-08-cost-performance-patterns.md), [6.9 Architecture Governance](../part-6-enterprise-architecture/chapter-09-architecture-governance.md).
- [P16 Multi-tenant GenAI Platform](../../projects/p16-multi-tenant-genai-platform/README.md) and [CS39](../../case-studies/cs39-internal-developer-copilot-platform.md) — the project and case study that build this catalog.
