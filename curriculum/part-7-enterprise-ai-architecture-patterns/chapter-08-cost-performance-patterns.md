...
2. State what each pattern trades away — latency, freshness, quality variance, interactivity — and price that trade before applying it.
...
4. Sequence the levers by risk rather than headline saving, and defend the order in a design review.
...
Each pattern carries one element beyond the standard form: **Measurement** — what to instrument *before* applying it, and the signal that says it is working.
...
- **Forces** — accuracy on genuinely hard classes against cost and latency variance on easy ones; the chain is usually summarized or withheld, so you cannot audit what you paid for; effort controls are provider-specific and shift across versions.
...
2. **Order levers by risk** — mechanical and quality-neutral first; those trading freshness or variance last, and never two at once.
...
5. **State the trade in the decision record** — what the pattern costs in latency, freshness, variance, or interactivity, beside the saving ([1.4](../part-1-professional-foundation/chapter-04-tradeoff-analysis.md)).
6. **Give every lever an owner and a cadence** — routing tables, thresholds, and budgets each decay on their own schedule.
