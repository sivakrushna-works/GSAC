...
- NFR-3 (Fairness): Disparity analysis by protected group at the operating thresholds, within policy bounds — with two realities stated up front: (a) in many non-mortgage lending regimes the lender *cannot collect* protected-class data, so analysis runs on proxy assignment (BISG-class methods), whose accuracy is itself contested and must be documented as a limitation; (b) adverse-impact ratios are one lens (imported from employment doctrine) — supervisors also use standardized mean differences and controlled regression, and "business necessity" is a legal burden-shifting test run with counsel, not a memo the modeling team writes ([2.8](../curriculum/part-2-artificial-intelligence/chapter-08-responsible-ai.md)).
